...
## 11. 계약 매출 일괄 생성 수동 점검

- 처리 대기 목록에서 현장·계약 검색, 현재 페이지 전체 선택, 검색 결과 최대 100건 선택이 동작하는지 확인
- 페이지를 이동해도 선택 계약이 유지되고 검색 조건을 바꾸면 선택과 미리보기가 초기화되는지 확인
- 미리보기에서 큐 순서, 계약별 처리 그룹, 품목별 신규·갱신·변경 없음·보호·자동 취소 결과와 금액을 확인
- 한 계약의 계약 버전을 바꾼 뒤 실행하면 해당 계약만 `계약 정보가 변경되었습니다`로 차단되고 나머지는 처리되는지 확인
- 한 계약을 월마감 처리한 뒤 실행하면 다른 계약은 성공하고 차단 계약만 재시도 대상으로 남는지 확인
- 성공 계약이 처리 대기에서 사라지고, 변경 없음·보호만 있는 계약도 큐에서 제거되는지 확인
- 반복 실행해도 `generatedKey` 중복 매출이 생기지 않고 batch ID와 계약별 감사 이력이 남는지 확인
...
- 여러 현장의 새 page 시작
- 신규 발행 후보가 현장·계약 구분·품목별로 나뉘고 각 행의 발행일을 따로 입력할 수 있는지 확인
- 동일 품목의 매출일이 여러 날이어도 한 문서로 묶이고 지정한 발행일이 저장되는지 확인
- 같은 현장·계약 구분·발행일의 품목이 발행 화면에서 하나의 자동 그룹과 합산 금액으로 표시되는지 확인
- 자동 그룹의 선택 품목을 별도 거래명세표로 분리하거나 자동 묶음으로 되돌릴 수 있는지 확인
- 같은 현장·계약 구분이라도 발행일이 다르면 별도 문서로 발행되고, 같은 발행일의 여러 품목은 한 문서의 여러 행으로 출력되는지 확인
- 일부 품목만 발행한 뒤 미발행 품목이 신규 후보로 남는지 확인
- 발행 이력에 `발행 품목`이 보이고 품목명 검색이 동작하는지 확인
- 인쇄물에 `발행 품목`이라는 별도 항목이 추가되지 않고 품목 행만 분리되어 보이는지 확인
- 발행일을 매출기간 밖으로 입력했을 때 미리보기 경고만 표시되고 발행은 가능한지 확인
- A4 margin, font, 잘림 여부
- 템플릿 수정·삭제 후 기존 발행본 재출력이 발행 당시 배치와 색상을 유지하는지 확인
- 미리보기 후 다른 사용자가 템플릿을 수정하면 발행이 중단되고 새 미리보기를 요구하는지 확인
- 월 중간 최초 발행 후 같은 현장·매출월의 추가 계약 매출을 생성·확정하고 대체 발행 미리보기에 기존분과 추가분이 모두 나오는지 확인
- 확정 매출이 없는 진행 계약이 있으면 누락 가능 계약 경고가 표시되는지 확인
- 대체 발행 후 새 번호의 문서만 `유효`, 이전 문서는 `대체됨`과 새 발행번호로 표시되는지 확인
- 대체된 과거본 재출력에 `대체된 과거 발행본` 안내가 표시되고 원래 행·합계·템플릿은 유지되는지 확인
- 첫 대체 발행 뒤 매출을 추가해 다시 대체 발행할 수 있는지 확인
- 조회 사용자는 대체 발행 버튼과 API를 사용할 수 없고 이력·재출력만 가능한지 확인
- 미리보기 후 다른 사용자가 먼저 대체 발행하거나 확정 매출을 변경하면 발행이 중단되고 새 미리보기를 요구하는지 확인
