...
- 발행 이력에서 유효 문서를 발행대기로 되돌릴 때 연결된 원본 매출 건수가 확인창에 표시되는지 확인
- 확정 매출이 없는 진행 계약이 있으면 누락 가능 계약 경고가 표시되는지 확인
- 되돌리기 후 기존 문서는 `취소됨`으로 보존되고 원본 매출은 개별 신규 후보로 다시 표시되는지 확인
- 되돌리기 전후 월마감과 확정 매출의 상태·금액이 바뀌지 않는지 확인
- 복원된 품목을 새 자동 묶음 또는 개별 분리로 지정해 새 번호로 발행할 수 있는지 확인
- 조회 사용자는 발행대기로 되돌리기 버튼과 API를 사용할 수 없고 이력·재출력만 가능한지 확인
- 현재 연결과 발행 이력이 불일치하거나 문서 버전이 바뀌면 전체 되돌리기가 중단되는지 확인
