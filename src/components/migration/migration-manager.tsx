...
function Summary({ label, value, detail, destructive = false }: { label: string; value: number; detail: string; destructive?: boolean }) {
  return <div className={"rounded-xl border bg-card p-4 " + (destructive ? "border-red-300" : "")}><p className="text-xs text-muted-foreground">{label}</p><p className={"mt-1 text-2xl font-semibold tabular-nums " + (destructive ? "text-red-700" : "")}>{value.toLocaleString()}</p><p className="mt-1 text-xs text-muted-foreground">{detail}</p></div>;
}
