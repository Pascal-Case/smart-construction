...
export default async function RevenuesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6"><div><p className="text-sm font-semibold text-teal-700">매출·청구</p><h1 className="text-2xl font-semibold">매출 원장</h1><p className="mt-1 text-sm text-muted-foreground">계약 자동 매출과 자유형·조정 매출을 건별 이력으로 관리합니다.</p></div><RevenueManager initialData={initialData} initialFilters={query} initialSort={initialSort} sites={sites} items={items} contractCategories={contractCategories} canEdit={user?.role === "ADMIN" || user?.role === "MANAGER"} /></div>;
}
