...
export default async function RevenuesPage() {
  ...
  const [result, sites, items, contracts] = await Promise.all([
    listRevenues({ q: "", startDate: "", endDate: "", siteId: "", sourceType: "all", status: "all", page: 1, pageSize: 20 }),
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6"><div><p className="text-sm font-semibold text-teal-700">매출·청구</p><h1 className="text-2xl font-semibold">매출 원장</h1><p className="mt-1 text-sm text-muted-foreground">계약 자동 매출과 자유형·조정 매출을 건별 이력으로 관리합니다.</p></div><RevenueManager initialData={initialData} sites={sites} items={items} contracts={contracts} canEdit={user?.role === "ADMIN" || user?.role === "MANAGER"} /></div>;
}
