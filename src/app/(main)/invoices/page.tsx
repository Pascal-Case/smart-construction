...
export default async function InvoicesPage({
  ...
  const [result, sites, company, templates, initialCandidates] = await Promise.all([
    ...
    initialMonth ? getInvoiceCandidates({ month: initialMonth, siteId: initialSiteId }) : Promise.resolve(null),
  ]);
  const initialData: InvoiceList = { ...result, rows: result.rows.map((row) => ({ ...row, issueDate: row.issueDate.toISOString(), periodStart: row.periodStart.toISOString(), periodEnd: row.periodEnd.toISOString(), issuedAt: row.issuedAt.toISOString(), updatedAt: row.updatedAt.toISOString(), supersededAt: row.supersededAt?.toISOString() ?? null })) };
  return <div className="mx-auto max-w-[1500px] space-y-6"><div><p className="text-sm font-semibold text-teal-700">청구·출력</p><h1 className="text-2xl font-semibold">거래명세표</h1><p className="mt-1 text-sm text-muted-foreground">마감한 현장·월 회차를 거래명세표로 발행하고 snapshot 발행본을 재출력합니다.</p></div><InvoiceManager initialData={initialData} sites={sites} templates={templates} canIssue={user?.role === "ADMIN" || user?.role === "MANAGER"} companyComplete={isCompanySettingComplete(company)} isAdmin={user?.role === "ADMIN"} initialMonth={initialMonth} initialSiteId={initialSiteId} initialCandidates={initialCandidates} /></div>;
}
