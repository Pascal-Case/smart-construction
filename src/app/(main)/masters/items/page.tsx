...
export default async function ItemsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [result, itemOptions] = await Promise.all([
    listItems(query),
    prisma.item.findMany({ select: { id: true, code: true, name: true, isActive: true, invoiceDisplayItemId: true }, orderBy: { name: "asc" } }),
  ]);
