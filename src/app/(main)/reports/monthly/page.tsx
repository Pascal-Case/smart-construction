...
export default async function MonthlyReportPage() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-6"><div><p className="text-sm font-semibold text-teal-700">월별 집계</p><h1 className="text-2xl font-semibold">월별 현황과 메모</h1><p className="mt-1 text-sm text-muted-foreground">현장별 청구액을 비교하고 금액 상세와 공유 메모를 확인합니다.</p></div><MonthlyReport initialData={initialData satisfies MonthlyReportData} sites={sites} items={items} canEdit={canEdit} currentUserId={user?.id ?? ""} /></div>;
}
