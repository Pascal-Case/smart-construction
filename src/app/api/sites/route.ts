...
import { siteInputSchema, siteListQuerySchema } from "@/lib/masters/schemas";
import { createSite, listSites } from "@/lib/masters/site-service";

export async function GET(request: Request) {
  try { await requireUser(); const url = new URL(request.url); return Response.json(await listSites(siteListQuerySchema.parse(Object.fromEntries(url.searchParams)))); }
