...
function parseItem(row: ParsedRow) {
  ...
  const aliases = aliasList(row.values["별칭"]); validateAliases(aliases, errors);
  const specification = optionalText(row.values["규격"]); const memo = optionalText(row.values["메모"]);
  validateLength(specification, 500, "규격", errors); validateLength(memo, 500, "메모", errors);
  return { code, name, specification, unit, errors, aliases, standardSalesPrice: moneyValue(row.values["표준매출단가"], "표준매출단가", errors), standardCostPrice: moneyValue(row.values["표준매입단가"], "표준매입단가", errors), isActive: booleanValue(row.values["사용여부"], errors), memo };
}
...
function mergeItem(row: ReturnType<typeof parseItem>, current?: { standardSalesPrice: number; standardCostPrice: number; isActive: boolean; specification: string | null; memo: string | null; aliases: Array<{ alias: string }> }) {
  return { code: row.code, name: row.name, specification: row.specification ?? current?.specification ?? null, unit: row.unit, standardSalesPrice: row.standardSalesPrice ?? current?.standardSalesPrice ?? 0, standardCostPrice: row.standardCostPrice ?? current?.standardCostPrice ?? 0, isActive: row.isActive ?? current?.isActive ?? true, memo: row.memo ?? current?.memo ?? null, aliases: row.aliases ?? current?.aliases.map((alias) => alias.alias) ?? [] };
}
...
function itemSnapshot(row: { code: string; name: string; specification: string | null; unit: string; standardSalesPrice: number; standardCostPrice: number; isActive: boolean; memo: string | null; aliases: Array<{ alias: string }> }) { return { code: row.code, name: row.name, specification: row.specification, unit: row.unit, standardSalesPrice: row.standardSalesPrice, standardCostPrice: row.standardCostPrice, isActive: row.isActive, memo: row.memo, aliases: row.aliases.map((a) => a.alias) }; }
