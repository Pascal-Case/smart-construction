...
Invoke-SqliteMaintenance -Operation verify -SourcePath $backup
...
try {
    Invoke-SqliteMaintenance -Operation backup -SourcePath $backup -DestinationPath $temporary
    if (-not (Test-Path -LiteralPath $temporary)) { throw "복구용 DB 복제에 실패했습니다." }
    if (Test-Path -LiteralPath $database) {
        & (Join-Path $PSScriptRoot "backup-database.ps1") -DatabasePath $database -BackupDirectory $PreRestoreBackupDirectory -RetentionDays 30 | Out-Null
    }
    foreach ($suffix in @("-wal", "-shm")) {
        $resolved = [IO.Path]::GetFullPath($database + $suffix)
    ...
    try {
        ...
        Invoke-SqliteMaintenance -Operation verify -SourcePath $database
        if (Test-Path -LiteralPath $previous) { Remove-Item -LiteralPath $previous -Force }
    } catch {
