...
- 기존 거래명세표의 발행 구성을 바꿀 때는 유효본을 취소 이력으로 보존하고 원본 매출을 발행대기로 복원한 뒤 새로 발행한다.
...
- `issueDate`
- `documentGroupKey`: 발행 화면에서 확정한 문서 그룹 snapshot. 발행대기로 복원할 때는 승계하지 않고 사용자가 새 그룹을 지정한다.
...
- 일부 품목만 먼저 발행하면 나머지 품목은 신규 후보로 남긴다. 유효 문서를 발행대기로 되돌리면 과거 link는 보존하고 현재 발행 연결만 원자적으로 해제해 원본 매출을 신규 후보로 복원한다.
...
1. 현장과 매출월을 선택하고 필요하면 계약 구분을 지정한 뒤 공통 발행일을 입력한다. 계약 구분의 기본값은 전체다.
