...
| 거래명세표 발행 단위 | 현장·월의 거래명세표는 계약 구분과 실제 원장 품목(`RevenueEntry.itemId`)별로 나누어 발행한다. 대표 출력 품목은 문서 경계를 바꾸지 않고 문서 내부 표시명·합산에만 적용한다. 품목이 없는 매출은 `품목 없음` 한 문서로 묶는다. |
| 계약 구분 | 코드 기반 관리자 마스터를 계약과 매출에 연결하고 월마감 snapshot에 고정한다. 한 현장·월의 거래명세표는 계약 구분별 문서로 나누고, 그 안에서 품목별 문서를 원자적으로 발행한다. |
...
- `issueDate`
- `issueItemId`, `issueItemName`: 발행 당시 품목 snapshot. 기존 다품목 문서는 `여러 품목`, 품목 미연결 문서는 `품목 없음`으로 보정한다.
...
- 같은 품목의 매출일이 달라도 사용자가 지정한 하나의 발행일로 같은 문서에 발행한다. 발행일이 매출기간 밖이면 미리보기에서 경고하되 발행은 허용한다.
- 일부 품목만 먼저 발행하면 나머지 품목은 신규 후보로 남긴다. 대체 발행은 선택한 문서의 현장·월·계약 구분 전체를 품목별 문서로 다시 만든다.
...
6. 인쇄 미리보기에서 문서가 품목별로 나뉘었는지와 발행일 경고를 확인한다. 발행 품목명은 관리 화면에만 표시한다.
7. 발행하면 문서·품목 snapshot과 문서별 발행일을 트랜잭션으로 저장한다.
8. 브라우저 인쇄 기능으로 방금 발행한 품목별 문서를 함께 출력한다.
...
- 현장·월·전체·개별 선택 출력
- 발행 snapshot과 발행번호
- 현장·계약 구분·실제 품목별 발행, 품목별 발행일, 부분 발행 후 잔여 후보 유지
