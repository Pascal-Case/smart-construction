...
| 거래명세표 합산 | 품목 마스터의 선택적 대표 출력 품목을 적용해 대표 품목별로 금액을 합산한다. 같은 문서에 대표 품목 매출이 있으면 그 매출의 규격·단위와 대표 수량 합계를 표시하고, 합산 공급가액을 대표 수량으로 원 단위까지 정확히 나눌 수 있을 때 계산 단가를 표시한다. 대표 품목 매출이 없으면 계산 열을 비운다. 대표 규격끼리 다르면 규격만 비우고, 단위가 다르면 수량·단위·단가를 비운다. 건별 표시는 원래 품목을 유지한다. |
| 거래명세표 발행 단위 | 현장·계약 구분·발행일을 기본 문서 그룹 키로 사용해 여러 품목을 한 문서의 여러 행으로 발행한다. 같은 자동 그룹 안에서는 사용자가 선택 품목을 별도 문서 그룹으로 분리할 수 있으며, 발행일이 다르면 자동으로 별도 문서가 된다. 대표 출력 품목은 문서 경계를 바꾸지 않고 문서 내부 표시명·합산에만 적용한다. |
| 계약 구분 | 코드 기반 관리자 마스터를 계약과 매출에 연결하고 월마감 snapshot에 고정한다. 한 현장·월의 거래명세표는 계약 구분별로 나누고, 최종 문서 그룹 키와 품목 snapshot을 발행 시점에 저장한다. |
...
- `issueDate`
- `documentGroupKey`: 발행 화면에서 확정한 문서 그룹 snapshot. 대체 발행에서도 같은 수동 분할을 유지한다.
- `issueItemId`, `issueItemName`: 발행 당시 품목 snapshot. 여러 품목 문서는 `여러 품목`, 품목 미연결 문서는 `품목 없음`으로 보정한다.
- `revenueFingerprint`: 문서에 연결된 원장·품목·대표 표시 설정 fingerprint
- `status`: `DRAFT | ISSUED`
...
10. 처리 대기 큐에서 계약 자동 매출을 현장·계약번호·계약명으로 검색하고 현재 페이지 또는 검색 결과 최대 100건을 선택할 수 있다.
11. 일괄 미리보기는 큐의 `pendingAt`, `contractId` 순서로 계약별 처리 그룹과 품목별 액션을 표시한다.
12. 일괄 생성은 계약별 transaction으로 처리하며 한 계약의 버전 충돌·월마감·권한·누락은 다른 계약의 성공을 되돌리지 않는다.
13. 미리보기 이후 계약 `version`이 바뀐 계약은 `CONTRACT_CHANGED`로 차단하고 새 미리보기를 요구한다.
14. 일괄 생성은 batch ID와 계약별 감사 로그를 남기며 성공·변경 없음·보호·차단 결과를 반환한다. 성공 계약은 큐에서 제거하고 차단 계약은 재시도 대상으로 남긴다.
...
- 계약 자동 매출 생성은 현재 페이지 전체 선택과 검색 결과 최대 100건 선택을 제공하고, 페이지 이동 중 선택을 유지한다
- 일괄 미리보기에서 계약별 처리 그룹·품목별 상세·예정 금액을 확인하며 계약별 부분 성공 결과와 재시도 대상을 표시한다
...
- 발행 후보를 현장·계약 구분·품목 단위로 선택하고 행별 발행일을 지정한다. 공통 발행일은 선택 행에 일괄 적용할 수 있다.
- 같은 현장·계약 구분·발행일의 선택 품목은 자동으로 하나의 문서 그룹으로 묶고, 관리 화면에서 현재 그룹과 예상 금액을 확인한다.
- 같은 자동 그룹의 선택 품목은 사용자가 별도 그룹으로 분리하거나 자동 묶음으로 되돌릴 수 있다. 발행일이 다르면 별도 문서 그룹으로 고정한다.
- 품목 마스터의 대표 출력 품목 설정은 문서 경계가 아니라 문서 내부 합산·표시행에만 적용한다. 발행일이 매출기간 밖이면 미리보기에서 경고하되 발행은 허용한다.
- 일부 품목만 먼저 발행하면 나머지 품목은 신규 후보로 남긴다. 대체 발행은 저장된 문서 그룹 키와 연결 매출을 기준으로 기존 수동 분할을 유지한다.
- 발행 이력에 발행 품목을 표시하고 품목명 검색을 지원한다. 인쇄물에는 발행 품목을 별도 표시하지 않는다.
- 실무 양식과 동일한 인쇄용 CSS
...
1. 현장과 매출월을 선택하고 공통 발행일을 입력한다.
2. 확정된 미발행 매출을 현장·계약 구분·품목 단위로 조회한다.
3. 원하는 품목 행을 체크하고 각 행의 발행일을 조정한다. 공통 발행일을 선택 행에 일괄 적용할 수도 있다.
4. 동일 품목 합산 또는 원장 건별 표시 방식을 선택한다.
5. 공급가액, VAT 10%, 합계를 계산한다. 기본 출력 양식에는 공급가액 합계와 `VAT 별도` 문구를 표시한다.
6. 발행일과 선택 상태를 기준으로 자동 문서 그룹을 확인하고, 필요하면 같은 자동 그룹의 선택 품목을 별도 그룹으로 분리한다.
7. 인쇄 미리보기에서 문서 그룹별 문서 수와 발행일 경고를 확인한다. 발행 품목명·그룹 상태는 관리 화면에만 표시한다.
8. 발행하면 문서 그룹·품목 snapshot과 문서별 발행일을 트랜잭션으로 저장한다.
9. 브라우저 인쇄 기능으로 방금 발행한 문서 그룹을 함께 출력한다.
...
- 현장·월·전체·개별 선택 출력
- 발행 snapshot과 발행번호
- 현장·계약 구분·발행일 자동 그룹, 그룹 내 다품목 발행과 수동 분리, 부분 발행 후 잔여 후보 유지
- 발행 이력의 발행 품목 표시와 기존 문서 migration 보정
- 발행본 재출력
